...
class UAV1Target1(Env):
    '''
    control uav with omega(w): turning rate
    '''
    ...
    def __init__(
        ...
        self.viewer = None
        self.SAVE_FRAMES_PATH = "../../../../visualized/1U1T_control_w" # example. save frames path is set at surveillance_PPO.py
    ...
    def step(self, action):
        ...
        if self.battery <= 0: # UAV dead
            ...
            if action == 3:  # go to charge uav1
                if (self.observation[0] < self.r_c):
                    ...
                    # self.uav1docked_time = 0
                    self.battery -= 1
                    self.w1_action = self.toc_get_action(self.observation[:2])
                    # if self.w1_action is numpy array, uncloth it:
                    self.w1_action = np.squeeze(self.w1_action)
                    self.w1_action = np.clip(self.w1_action, -self.omega_max, self.omega_max)
                    self.uav1kinematics(self.w1_action)
            elif action == 4:  # surveil target1
                self.battery -= 1
                self.w1_action = self.dkc_get_action(self.rel_observation[:2])
                self.w1_action = np.squeeze(self.w1_action)
                self.w1_action = np.clip(self.w1_action, -self.omega_max, self.omega_max)
                self.uav1kinematics(self.w1_action)
            elif action == 0:  # no change in w
                self.battery -= 1
                self.uav1kinematics(self.w1_action)
            elif action == 1:
                self.battery -= 1
        ...
        if self.save_frames:
            if int(self.step_count) % 6 == 0:
                ...
                # left upper corner
                text0 = f"\u03C9: {self.w1_action}"
                text1 = "battery: {}".format(self.battery)
                text2 = "age: {}".format(self.age)
                text3 = "Reward: {}".format(reward)
                draw.text((0, 0), text0, color=(200, 200, 200), font=self.font)
                draw.text((0, 30), text1, color=(200, 200, 200), font=self.font)
                draw.text((0, 60), text2, color=(255, 255, 0), font=self.font)
                draw.text((0, 90), text3, color=(200, 200, 200), font=self.font)
                image.save(path)
            ...
            if battery_copy <= 0:  # UAV dead
                ...
                if action == 0:
                    if (dry_dict_observation['uav1_charge_station'][0] < self.r_c):
                        ...
                        battery_copy -= 1
                        self.w1_action = self.toc_get_action(dry_dict_observation['uav1_charge_station'][:2])
                        uav1_state_copy = self.dry_uav1kinematics(self.w1_action, uav1_state_copy)
                    
                else:
                    battery_copy -= 1
                    self.w1_action = self.dkc_get_action(dry_dict_observation['uav1_target1'][:2])
                    uav1_state_copy = self.dry_uav1kinematics(self.w1_action, uav1_state_copy)
            # Dry observation
    ...
    def render(self, action, mode="human", control=False):
        ...
        draw = ImageDraw.Draw(image)
        # left upper corner
        text0 = f"\u03C9: {self.w1_action/self.omega_max:.2f}"
        # convert to %
        text1 = f"battery: {self.battery/3000*100:.0f}%"
        text2 = "age: {}".format(self.age)
        draw.text((0, 0), text0, color=(200, 200, 200), font=self.font)
