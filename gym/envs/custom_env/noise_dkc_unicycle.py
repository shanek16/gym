...
class Noise_DKC_Unicycle(Env):
    ...
    def controller(self, r, alpha):
        rdot = -self.v * np.cos(alpha)
        direction = np.sign(alpha)
        if r == 0:
            uNav = self.omega_max
        else:
            uNav = (1/np.sqrt(self.v**2 - rdot**2))*(self.k1*(r-self.d)+(self.v**2 - rdot**2)/r + self.k2*rdot)

        omegaRef = direction*uNav
